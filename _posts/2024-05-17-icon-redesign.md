...
It's been a while since we last posted an update here, but we're back with some exciting news. We've been working hard on Pencil2D, and we'd like to showcase a complete redesign of our icon set that will be included in the next version:

| Old | New |
| --- | --- |
| ![Screenshot of Pencil2D with the old icon set]({{ "/images/icon-redesign-old.png" | relative_url }}){:width="1343"} | ![Screenshot of Pencil2D with the new icon set]({{ "/images/icon-redesign-new.png" | relative_url }}){:width="1343"} |
...
To see the new icons in action now, please [download the experimental nightly builds from our website](/download/nightly/). Besides the icons, they also come with many other exciting improvements that will be part of the next version!

In addition to the icons themselves, MrStevns has written a comprehensive overview that explains the rationale behind the new icon design, the color palette, and how to create new icons for Pencil2D. We encourage you to check it out and get inspired to contribute to the creative process of Pencil2D.
...
With that in mind, we settled on the following principles for the new icon set:
...
The inspiration comes from [Affinity Designer]({{ "/images/icon-redesign-color-grouping-affinity-designer.png" | relative_url }}), which does the same.
...
Besides designing the icons in the first place, we still needed to make those SVG icons look good in the context of our Qt application. In this section we'll go over optimization techniques, rules, limitations of the Qt SVG support, and export considerations.
...
So in order to always get a good-looking result, the trick is to pick the lowest resolution you want to support, let's say 22×22, which is the case for all our icons. We need to work within the limitations of the canvas, and whenever possible, stay on whole pixels, i.e., x: 1, y: 2, rather than x: 0.4, y: 0.7.
...
In my experience, using groups, clipping bounds, and effects like shadows, glow, outline, and raster images is not supported by Qt's SVG module, so using those has to be avoided. Instead, we need to be smart and think about how we can merge edges, pixels that overlap, and things that need to be within bounds of each other.
...
Like this new icon set, all improvements to Pencil2D are developed by passionate volunteers in their spare time. You can also become a volunteer and contribute in various ways, not just through graphic design or programming! Please check out the [Contribute page](/contribute/) to learn more.
